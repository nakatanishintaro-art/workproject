...
# 最低保証件数（スコアに関わらず必ずこの件数を通知する）
MIN_NOTIFY_COUNT = int(os.getenv("MIN_NOTIFY_COUNT", "10"))
...
def get_top_jobs(session, min_score: float = 60, min_count: int = 10) -> list[dict]:
    """
    通知対象案件を取得する。
    スコア閾値以上の未通知案件を優先し、それが min_count 件未満の場合は
    スコア順に補完して必ず min_count 件を返す。
    """
    def job_to_dict(j):
        return {
            "job_id": j.job_id,
    ...
    result = [job_to_dict(j) for j in high_score_jobs]

    # min_count に満たない場合、スコア順で補完（閾値未満・通知済みも含む）
    if len(result) < min_count:
        existing_ids = {j["job_id"] for j in result}
        additional_jobs = (
            session.query(Job)
            .filter(
                Job.job_id.notin_(existing_ids),
                Job.is_active == True,
                Job.ai_score.isnot(None),
            )
            .order_by(Job.ai_score.desc())
            .limit(min_count - len(result))
            .all()
        )
        result += [job_to_dict(j) for j in additional_jobs]

        if result:
            logger.info(
                f"[Main] スコア{min_score}点以上の新規案件が{len(high_score_jobs)}件のみのため、"
                f"スコア順で補完して合計{len(result)}件を通知します。"
            )
...
def run_full_pipeline():
    ...
    try:
        # 1. スクレイピング
        logger.info("[Main] Step 1: スクレイピング開始")
        ...
        # 4. 通知対象案件を取得（スコア閾値以上 + 最低MIN_NOTIFY_COUNT件を保証）
        min_score = int(os.getenv("MIN_SCORE_TO_NOTIFY", "60"))
        recommended = get_top_jobs(session, min_score, MIN_NOTIFY_COUNT)
        ...
        if os.getenv("SLACK_WEBHOOK_URL"):
            ...
            if success:
                # 新規の高スコア案件のみ通知済みマーク
                new_high_score_ids = [
                    j["job_id"] for j in recommended
                    if j.get("ai_score", 0) >= min_score
                ]
                mark_as_notified(session, new_high_score_ids)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="案件自動収集・AIマッチングシステム")
